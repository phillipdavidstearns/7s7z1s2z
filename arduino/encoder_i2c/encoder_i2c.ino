...
ISR(INT0_vect) { //pin change interrupt
  ...
  volatile boolean encA = (PORTB >> ENCA_PIN) & 1; //triggered the interrupt
  volatile boolean encB = (PORTB >> ENCB_PIN) & 1;
...
int onI2CReceive() {
  ...
  while (TinyWire.available() > 0) {
    ...
    if (data == 'g') { // "get" request, send counter value
      uint8_t count[] = { (counter >> 24) & 0xff, (counter >> 16) & 0xff, (counter >> 8) & 0xff, counter & 0xff };
      TinyWire.send(count,4);
